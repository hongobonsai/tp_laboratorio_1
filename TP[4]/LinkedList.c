	...
	if (this != NULL && nodeIndex >= 0 && nodeIndex <= ll_len(this)) {
		if ((pNewNode = (Node*) malloc(sizeof(Node))) != NULL) {
			if (nodeIndex == 0) {
				...
				if ((pNodeAux = getNode(this, nodeIndex - 1)) != NULL) {
	...
	if (this != NULL && index >= 0 && index < ll_len(this)) {
		if ((pNodeAux = getNode(this, index)) != NULL) {
			if (index == 0) {
				...
				if ((pNodeAuxDos = getNode(this, index - 1)) != NULL) {
...
void* ll_pop(LinkedList *this, int index) {
	void *pDeletedElement = NULL;
	if(this != NULL && index >= 0 && index <= ll_len(this)){
		pDeletedElement = ll_get(this, index);
			ll_remove(this, index);
		}

	return pDeletedElement;
}
